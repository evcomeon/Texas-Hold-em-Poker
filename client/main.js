...
function renderPlayers() {
  ...
  orderedPlayers.forEach((player, i) => {
    ...
    const isMe = i === 0 && !isSpectator;
    let html = `<div class="flex flex-col items-center relative ${isCurrent ? 'ring-4 ring-secondary rounded-xl scale-110 transition-transform' : ''} ${player.folded || player.disconnected ? 'opacity-50 grayscale' : ''}">`;
    ...
    if (isMe) {
        ...
        if (player.holeCards && player.holeCards.length > 0) {
            player.holeCards.forEach((card, idx) => {
                ...
                const rotateCls = idx === 0 ? '-rotate-6' : 'rotate-6';
                if(card.hidden) {
                   html += `<div class="w-12 h-16 sm:w-20 sm:h-28 rounded-lg shadow-2xl flex flex-col items-center justify-center p-2 sm:p-3 border border-outline-variant bg-surface-container-highest transform ${rotateCls}">`;
                   html += `<div class="w-full h-full bg-gradient-to-br from-primary/10 to-transparent flex items-center justify-center"><span class="material-symbols-outlined text-primary/40 text-2xl sm:text-4xl">style</span></div>`;
                   html += `</div>`;
                } else {
                   html += `<div class="w-12 h-16 sm:w-20 sm:h-28 bg-white rounded-lg shadow-2xl flex flex-col justify-between p-2 sm:p-3 border-2 border-primary ring-4 ring-primary/20 transform ${rotateCls}">`;
                   const txtColor = isRed ? 'text-red-600' : 'text-black';
                   const icon = card.suit === '♥' ? 'favorite' : card.suit === '♦' ? 'diamond' : card.suit === '♠' ? 'spades' : 'club';
                   html += `<div class="${txtColor} font-headline font-bold text-base sm:text-xl leading-none">${card.rank}<br/><span class="text-xs sm:text-sm">${card.suit}</span></div>`;
                   html += `<div class="self-center material-symbols-outlined ${txtColor} text-2xl sm:text-4xl" style="font-variation-settings: 'FILL' 1;">${icon}</div>`;
                   html += `<div class="${txtColor} font-headline font-bold text-base sm:text-xl leading-none rotate-180">${card.rank}<br/><span class="text-xs sm:text-sm">${card.suit}</span></div>`;
                   html += `</div>`;
                }
            });
        }
        html += `</div>`;
        html += `<div class="glass-panel px-4 sm:px-6 py-2 sm:py-3 rounded-xl border-2 border-primary ring-4 sm:ring-8 ring-primary/10 text-center relative z-20">`;
        html += `<p class="text-[10px] sm:text-xs font-bold uppercase text-primary tracking-widest mb-1">${player.name} (You)</p>`;
        html += `<p class="text-secondary text-lg sm:text-xl font-headline font-black leading-none">${player.chips.toLocaleString()}</p>`;
        html += `</div>`;
    } else {
        html += `<div class="relative w-12 h-12 sm:w-16 sm:h-16 rounded-full border-2 border-outline-variant bg-surface-container-low overflow-hidden shadow-xl">`;
        html += `<img src="${player.picture || `https://api.dicebear.com/7.x/avataaars/svg?seed=${player.name}`}" class="w-full h-full object-cover">`;
        html += `</div>`;
        html += `<div class="glass-panel mt-1 sm:mt-2 px-2 sm:px-3 py-1 rounded-lg text-center relative z-30">`;
        html += `<p class="text-[8px] sm:text-[10px] font-bold uppercase truncate w-16 sm:w-24 text-on-surface">${player.name}</p>`;
        html += `<p class="text-secondary text-[10px] sm:text-xs font-headline font-black">${player.chips.toLocaleString()}</p>`;
        html += `</div>`;
        html += `<div class="flex gap-1 -mt-3 sm:-mt-4 z-20">`;
        if (player.holeCards && player.holeCards.length > 0) {
            player.holeCards.forEach(card => {
                if (card.hidden) {
                    html += `<div class="w-5 h-7 sm:w-6 sm:h-9 bg-primary-container rounded shadow-md border border-primary/20"></div>`;
                } else {
                    const isRed = card.suit === '♥' || card.suit === '♦';
                    const txtColor = isRed ? 'text-red-600' : 'text-black';
                    html += `<div class="w-5 h-7 sm:w-6 sm:h-9 bg-white rounded shadow-md border border-outline-variant flex flex-col items-center justify-center -space-y-1 p-0.5">`;
                    html += `<span class="${txtColor} font-bold text-[8px] sm:text-[10px] leading-none">${card.rank}</span>`;
                    html += `<span class="${txtColor} text-[8px] sm:text-[10px] leading-none">${card.suit}</span>`;
                    html += `</div>`;
                }
            });
        }
    ...
    if (player.isDealer) {
        html += `<div class="absolute -right-2 top-0 bg-secondary rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center border-2 border-surface shadow-lg z-50">`;
        html += `<span class="text-[8px] sm:text-[10px] font-black text-on-secondary">D</span></div>`;
    }
    
    let statusText = '';
    let statusColor = 'text-warning';
    if (player.disconnected) { statusText = 'Disconnected'; statusColor = 'text-on-surface-variant'; }
    else if (player.folded) { statusText = 'Folded'; statusColor = 'text-on-surface-variant'; }
    else if (player.allIn) { statusText = 'ALL-IN'; statusColor = 'text-secondary'; }
    
    if (statusText) {
       html += `<div class="absolute inset-0 flex items-center justify-center z-50 pointer-events-none mt-10">`;
       html += `<span class="bg-surface-container-highest px-2 py-0.5 rounded text-[8px] font-bold ${statusColor} uppercase border border-outline-variant/50">${statusText}</span>`;
       html += `</div>`;
    }

    if (player.bet > 0) {
       html += `<div class="absolute ${isMe ? '-top-6' : '-bottom-6'} w-max bg-surface-container-highest px-2 py-0.5 rounded-full border border-secondary/20 flex items-center gap-1 shadow-lg z-50">`;
       html += `<span class="material-symbols-outlined text-secondary text-[10px]" style="font-variation-settings: 'FILL' 1;">monetization_on</span>`;
       html += `<span class="text-[10px] font-bold text-secondary">${player.bet.toLocaleString()}</span>`;
       html += `</div>`;
    }
    
    if (isWinner) {
       html += `<div class="absolute top-0 right-0 w-max bg-primary-container px-2 py-1 rounded text-[10px] font-bold text-on-primary-container z-50 animate-bounce">+${wonAmount.toLocaleString()}</div>`;
    }
    
    if (player.bestHand && isShowdown && !player.folded) {
       html += `<div class="absolute bottom-[-15px] sm:bottom-[-20px] text-[8px] sm:text-[10px] bg-secondary/80 text-background px-1 rounded font-bold">${player.bestHand}</div>`;
    }
...
function renderCommunityCards() {
  ...
  if (gameState.communityCards && gameState.communityCards.length > 0) {
    for (const card of gameState.communityCards) {
      ...
      const txtColor = isRed ? 'text-red-600' : 'text-black';
      const icon = card.suit === '♥' ? 'favorite' : card.suit === '♦' ? 'diamond' : card.suit === '♠' ? 'spades' : 'club';
      html += `<div class="w-10 h-16 sm:w-16 sm:h-24 bg-white rounded-lg shadow-lg flex flex-col justify-between p-1 sm:p-2 border border-outline-variant/20">`;
      html += `<div class="${txtColor} font-headline font-bold text-sm sm:text-lg leading-none">${card.rank}<br/><span class="text-[8px] sm:text-xs">${card.suit}</span></div>`;
      html += `<div class="self-center material-symbols-outlined ${txtColor} text-xl sm:text-3xl" style="font-variation-settings: 'FILL' 1;">${icon}</div>`;
      html += `<div class="${txtColor} font-headline font-bold text-sm sm:text-lg leading-none rotate-180">${card.rank}<br/><span class="text-[8px] sm:text-xs">${card.suit}</span></div>`;
      html += `</div>`;
    }
  }
...
function renderPot() {
  els.potAmount.textContent = (gameState.pot || 0).toLocaleString();
}
...
function renderActions() {
  ...
  if (actions.includes('nextHand')) {
    html += `<button class="flex-1 max-w-[200px] py-4 rounded-lg bg-gradient-to-br from-primary to-primary-container text-on-primary font-headline font-black uppercase tracking-widest shadow-xl active:scale-95 transition-transform" onclick="window._nextHand()">准备 ${gameState.readyRemainingTime > 0 ? `(${gameState.readyRemainingTime}s)` : ''}</button>`;
    html += `<button class="flex-1 max-w-[120px] py-4 rounded-lg bg-surface-container-highest border border-outline-variant text-on-surface font-headline font-bold uppercase tracking-wider hover:bg-outline-variant transition-colors" onclick="window._backToLobby()">离开</button>`;
  ...
  if (actions.length === 0) {
    if (gameState.isSpectator) {
      html += `<div class="text-on-surface-variant text-xs font-bold uppercase tracking-widest px-4">观战中</div>`;
      html += `<button class="max-w-[120px] px-6 py-2 rounded-lg bg-surface-container-highest text-on-surface hover:bg-outline-variant transition-colors uppercase font-bold text-xs" onclick="window._backToLobby()">返回大厅</button>`;
    } else {
      html += `<div class="text-on-surface-variant text-sm font-bold uppercase tracking-widest animate-pulse">Wait holding...</div>`;
    }
    els.actionButtons.innerHTML = html;
    els.raiseControls.classList.add('hidden');
    els.actionButtons.classList.remove('hidden');
  ...
  if (actions.includes('fold')) {
    html += `<button class="flex-1 max-w-[120px] py-3 sm:py-4 rounded-lg bg-surface-container-highest border border-outline-variant text-on-surface font-headline font-bold uppercase tracking-wider hover:bg-outline-variant transition-colors active:scale-95 text-xs sm:text-base" onclick="window._action('fold')">Fold</button>`;
  }
  if (actions.includes('check')) {
    html += `<button class="flex-1 max-w-[120px] py-3 sm:py-4 rounded-lg bg-surface-container-highest border border-outline-variant text-on-surface font-headline font-bold uppercase tracking-wider hover:bg-outline-variant transition-colors active:scale-95 text-xs sm:text-base" onclick="window._action('check')">Check</button>`;
  }
  if (actions.includes('call')) {
    ...
    html += `<button class="flex-1 max-w-[200px] py-3 sm:py-4 rounded-lg bg-gradient-to-br from-primary to-primary-container text-on-primary font-headline font-extrabold uppercase tracking-widest shadow-lg shadow-primary/20 active:scale-95 transition-transform text-xs sm:text-base" onclick="window._action('call')">Call ${callAmount}</button>`;
  }
  if (actions.includes('raise')) {
    html += `<button class="flex-1 max-w-[140px] py-3 sm:py-4 rounded-lg bg-gradient-to-br from-secondary to-secondary-container text-on-secondary-container font-headline font-black uppercase tracking-widest shadow-xl shadow-secondary/10 active:scale-95 transition-transform text-xs sm:text-base" onclick="window._toggleRaise()">Raise</button>`;
  }
  if (actions.includes('allin')) {
    html += `<button class="flex-1 max-w-[140px] py-3 sm:py-4 rounded-lg bg-gradient-to-br from-error to-error-container text-on-error font-headline font-black uppercase tracking-widest shadow-xl shadow-error/10 active:scale-95 transition-transform text-xs sm:text-base" onclick="window._action('allin')">All-In</button>`;
  }
...
function setupEventListeners() {
  // Login tabs
  document.querySelectorAll('.login-tabs button[data-tab]').forEach(tab => {
    tab.addEventListener('click', () => {
      document.querySelectorAll('.login-tabs button[data-tab]').forEach(t => {
        t.classList.remove('text-primary', 'border-primary');
        t.classList.add('text-on-surface-variant', 'border-transparent');
      });
      ['guest', 'login', 'register'].forEach(id => {
